...
class Guide(PyroModule):
    ...
    def __init__(self, config):
        ...
        if config["num_layers"] > 1:
            ...
            self.y_nn = lambda x:x
            self.y0_nn = OutcomeNet([config["feature_dim"]])
            self.y1_nn = OutcomeNet([config["feature_dim"]])
        ...
        if config["num_layers"] > 1:
            ...
            self.z_nn = lambda z:z
            self.z0_nn = DiagNormalNet([config["feature_dim"]+1, config["latent_dim"]])
            self.z1_nn = DiagNormalNet([config["feature_dim"]+1, config["latent_dim"]])            
