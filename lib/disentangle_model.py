...
from dist import UnnormalizedExponentialFamily 
...
class FullyConnected(nn.Sequential):
    ...
    def __init__(self, sizes, activation="ELU", final_activation=None):
        layers = []
...
class nfI2VAE_Model(PyroModule):
    """
    Generative model for a causal model with latent variables ``z``, which
    are composed of latent confounders and mediators and binary treatment ``t``::
        z ~ p(z|t,y)   # latent variables
        x ~ p(x|z)     # features
    """
    ...
    def __init__(self, config):
        ...
        self.t_nf = FullyConnected([config["latent_dim"], 1])
        self.lambd_f = FullyConnected([2, 2*config["latent_dim"]])
        self.lambd_nf = FullyConnected([2, 1])

    def forward(self, x, t=None, y=None, size=None):
        if size is None:
            size = x.size(0)
        with pyro.plate("data", size, subsample=x):
            z = pyro.sample("z", self.z_dist(t, y))
    ...
    def z_dist(self, t, y):
        ...
        lambd_f = self.lambd_f(torch.cat([t, y], axis=-1))
        lambd_nf = self.lambd_nf(torch.cat([t, y], axis=-1))
        return UnnormalizedExponentialFamily(self.t_f, self.t_nf, lambd_f, lambd_nf)
...
class nfI2VAE_Guide(PyroModule):
    """
    Inference model for latent confounder and mediators upto element-wise 
    transformation and permutation::
        z ~ q(z|y,t,x)  # latent variables, an embedding
    """

    def __init__(self, config):
        self.latent_dim = config["latent_dim"]
        super().__init__()
        self.z_nn1 = FullyConnected(
            [2 + config["feature_dim"]]
            + [config["hidden_dim"]] * (config["num_layers"] - 1),
            final_activation=nn.ELU(),
        )
        self.z_nn2 = DiagNormalNet([config["hidden_dim"], config["latent_dim"]])
    ...
    def z_dist(self, y, t, x):
        ...
        in_ = torch.cat([y.unsqueeze(-1), t.unsqueeze(-1), x], dim=-1)
        params = self.z_nn2(self.z_nn1(in_))
        return dist.Normal(*params).to_event(1)

    def z_mean(self, x, t, y):
        return self.z_dist(y, t, x).mean


class nfI2VAE(nn.Module):
    """
    Main class implementing the nfI2VAE
    """
    ...
    def __init__(
        ...
        super().__init__()
        self.model = nfI2VAE_Model(config)
        self.guide = nfI2VAE_Guide(config)
    ...
    @torch.no_grad()
    def infer(self, x, t, y):
        r"""
            Infer the latent variable associated with sample x, t
        """
        ...
        with pyro.plate("num_particles", num_samples, dim=-2):
            with poutine.trace() as tr:
                self.guide(x, t, y)
        z = poutine.replay(self.guide.z_mean, tr.trace)(x, t, y)
...
class Model(PyroModule):
    ...
    def y_mean(self, x, t=None):
        with pyro.plate("data", x.size(0)):
            z = pyro.sample("z", self.z_dist_dummy())
            x = pyro.sample("x", self.x_dist(z), obs=x)
            t = pyro.sample("t", self.t_dist(z), obs=t)
        return self.y_dist(t, z).mean

    def z_dist_dummy(self):
...
class Guide(PyroModule):
    ...
    def z_mean(self, x, t=None, y=None):
        with pyro.plate("data", x.size(0)):
